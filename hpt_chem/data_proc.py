...
- Crops (ZED RGB,D), downsamples(Z RGB,D, W RGB) and runs ResNet on RGB

- Saves everything at 15hz for now

hdf5 structure:

root {ep#, [source_names]}
|  episode_0
|  |  data
|  |  |  source_0
|  |  |  source_1
...
with h5py.File(f"{TARGET_CHEMDATA_PATH}/{DATASET_NAME}.hdf5", "w") as dataset:
    files = os.listdir(SOURCE_CHEMDATA_FOLDER_PATH) # all files

"""Convert raw chemdata recordings into a LeRobot dataset 
...
from lerobot.datasets import LeRobotDataset, split_dataset
...
def locf_align(source_stamps: np.ndarray, grid_ms: np.ndarray) -> np.ndarray:
    #LOCF-align one source onto the uniform ms grid
...
def main() -> None:
    ...
    dataset = LeRobotDataset.create(
        repo_id=REPO_ID,
        root=f"{TARGET_CHEMDATA_PATH}/{DATASET_NAME}",
        fps=HZ_PER_SOURCE,
        features=FEATURES,
    )

    files = sorted(f for f in os.listdir(SOURCE_CHEMDATA_FOLDER_PATH) if f.endswith(".h5"))
    print(f"[INFO]: {len(files)} raw episodes found in {SOURCE_CHEMDATA_FOLDER_PATH}")
